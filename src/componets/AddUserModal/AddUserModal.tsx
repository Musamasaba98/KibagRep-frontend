        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          {!selected ? (
            <div>
              ...
              {!searching && results.length === 0 && q.length >= 2 && (
                <p className="text-xs font-poppins text-gray-400 mt-2">No users found for "{q}"</p>
              )}
              {results.length > 0 && (
          ...
          <div>
            ...
            <select value={role} onChange={(e) => setRole(e.target.value)}
              className="w-full px-3.5 py-2.5 font-poppins border border-gray-200 rounded-xl text-sm outline-none focus:border-[#16a34a] focus:ring-2 focus:ring-[#16a34a]/20 bg-white">
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={onClose}
              className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-poppins-semibold text-gray-600 hover:bg-gray-50 focus-visible:outline-none">
              Cancel
            </button>
            <button onClick={handleAdd} disabled={saving || !selected || !role}
              className="flex-1 py-2.5 rounded-xl bg-[#16a34a] hover:bg-[#15803d] text-white text-sm font-poppins-bold disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#16a34a]"
