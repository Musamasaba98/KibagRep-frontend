...
interface Activity {
  ...
  user: { id: string; firstname: string; lastname: string; role: string };
  doctor: { id: string; doctor_name: string; speciality?: string[]; location?: string; town: string };
...
const RepDetail = () => {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (feedRes.status === "fulfilled") {
        ...
        setActivities(all.filter((a) => a.user.id === id).sort(
          (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
        ));
      }
      if (reportsRes.status === "fulfilled") {
        const all: Report[] = reportsRes.value.data?.data ?? [];
        setReports(all.filter((r) => r.user.id === id).sort(
  ...
  return (
    <div className="w-full p-6 flex flex-col gap-6 max-w-5xl mx-auto">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* Visit activity — takes 2 columns */}
                    ...
                    <div key={act.id} className="flex items-start gap-3 px-5 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {act.gps_anomaly && (
                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-red-100 text-red-600 border border-red-200 flex items-center gap-0.5">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          ...
                          {act.nca_reason && (
                            <> · <span className="text-amber-600">{act.nca_reason}</span></>
                          )}
                        </p>
                      </div>
